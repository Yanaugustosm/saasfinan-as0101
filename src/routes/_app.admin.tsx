...
function LoginScreen({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center px-4 relative"
      ...
      <div className="relative w-full max-w-[400px] rounded-2xl border p-8"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? <span className="flex items-center justify-center gap-2"><i className="ti ti-loader-2 animate-spin" />Verificando…</span> : "Entrar no Painel"}
          </button>
        </form>
      ...
      <div className="mt-6 text-[11px] tracking-[0.2em] uppercase" style={{ color: C.dim }}>Sincronia SaaS</div>
    </div>
  );
}
...
function Dashboard({ onLogout }: { onLogout: () => void }) {
  ...
  const growthData = useMemo(() => {
    ...
    let acc = 0;
    return Array.from(map.entries()).sort(([a], [b]) => a.localeCompare(b)).map(([date, n]) => {
      acc += n;
  ...
  const exportUsersCSV = () => {
    const headers = ["Nome", "E-mail", "Grupo", "Cadastrado em", "Status"];
    ...
    downloadCSV([headers, ...rows], `sincronia-usuarios-${new Date().toISOString().slice(0, 10)}.csv`);
    showToast(`${filteredUsers.length} usuários exportados.`);
  };
  const exportGroupsCSV = () => {
    const headers = ["Grupo", "Emoji", "Membros", "Código", "Criado em"];
    const rows = filteredGroups.map(g => [g.groupName, g.groupEmoji ?? "", String(g.members?.length ?? 0), g.inviteCode ?? "—", g.createdAt?.slice(0, 10) ?? "—"]);
    downloadCSV([headers, ...rows], `sincronia-casais-${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <div className="w-full h-full flex" style={{ background: C.bg }}>
      ...
      {/* ── Toast ── */}
      {toast && (
        <div className="fixed top-4 right-4 z-[700] px-4 py-3 rounded-xl border text-[13px] font-medium backdrop-blur-sm"
          style={{ background: toast.ok ? "rgba(116,185,101,0.1)" : "rgba(239,68,68,0.1)", borderColor: toast.ok ? "rgba(116,185,101,0.3)" : "rgba(239,68,68,0.3)", color: toast.ok ? C.green : C.red }}>
          {toast.msg}
        </div>
      )}
      ...
      {confirmBulkDel && (
        <Modal>
          ...
          <p className="text-[13px] mb-6" style={{ color: C.muted }}>
            Excluir permanentemente <strong className="text-white">
              {confirmBulkDel === "user" ? `${selUsers.size} usuário(s)` : `${selGroups.size} casal(is)`}
            </strong> do banco de dados?
          </p>
          <div className="flex gap-3">
            <ModalBtn label="Cancelar" onClick={() => setConfirmBulkDel(null)} />
            <ModalBtn label={`Excluir ${confirmBulkDel === "user" ? selUsers.size : selGroups.size} item(s)`} onClick={doBulkDelete} danger />
          </div>
        </Modal>
      )}
      ...
      <aside className="flex-shrink-0 flex flex-col h-full" style={{ width: 256, background: C.surface, borderRight: `1px solid ${C.border}` }}>
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          ...
          {navItems.map(({ id, icon, label }) => {
            const active = tab === id;
            return (
              <button key={id} onClick={() => { setTab(id); setSelUsers(new Set()); setSelGroups(new Set()); }}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Topbar */}
        <div className="flex-shrink-0 flex items-center justify-between px-8 h-14" style={{ background: C.surface, borderBottom: `1px solid ${C.border}` }}>
          <div className="flex items-center gap-3">
        ...
        <div className="flex-1 overflow-y-auto p-8 relative">
              ...
              <div className="rounded-2xl border overflow-hidden" style={{ background: C.card, borderColor: C.border }}>
                ...
                {loading
                  ...
                  : users.slice(0, 5).map((u, i) => (
                    <div key={u.uid} className="flex items-center gap-4 px-6 py-3.5 cursor-pointer transition hover:bg-white/[0.02]"
          ...
          {tab === "users" && (
            <div className="space-y-5">
              ...
              {selUsers.size > 0 && (
                <BulkActionBar
                  count={selUsers.size}
                  onDelete={() => setConfirmBulkDel("user")}
          ...
          {tab === "groups" && (
            <div className="space-y-5">
              ...
              {selGroups.size > 0 && (
                <BulkActionBar
                  count={selGroups.size}
                  onDelete={() => setConfirmBulkDel("group")}
                  onClear={() => setSelGroups(new Set())}
                />
              )}
                  ...
                  : filteredGroups.map((g, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[11.5px] font-mono px-2 py-0.5 rounded-md w-fit" style={{ color: C.gold, background: "rgba(201,169,110,0.08)" }} onClick={() => setDrawerGroup(g)}>{g.inviteCode}</span>
...
function UserSideDrawer({
  user, groups, onClose, onSave, onToggleSuspend, onDelete,
}: {
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className="fixed inset-0 z-[550] transition-opacity"
        style={{ background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)", opacity: user ? 1 : 0, pointerEvents: user ? "auto" : "none" }}
        onClick={onClose}
      />
      {/* Drawer */}
      <div
        className="fixed top-0 right-0 h-full z-[560] flex flex-col overflow-y-auto"
        style={{
          width: 420,
          background: "#0f0f15",
        ...
        {local && (
          <>
            ...
            <div className="flex items-center justify-between px-6 pt-6 pb-5" style={{ borderBottom: `1px solid ${C.border}` }}>
              ...
              <button onClick={onClose} className="size-8 rounded-lg flex items-center justify-center transition" style={{ color: C.muted, background: "rgba(255,255,255,0.04)" }}>
            ...
            <div className="flex-1 px-6 py-5 space-y-6">
              ...
              <div>
                ...
                <div className="space-y-3 mt-3">
                  <DField label="Nome">
                    <DInput value={local.name} onChange={v => setLocal({ ...local, name: v })} />
                  </DField>
                  <DField label="E-mail">
                    <DInput value={local.email} onChange={v => setLocal({ ...local, email: v })} />
                  </DField>
                  <DField label="Emoji / Avatar">
                    <DInput value={local.emoji} onChange={v => setLocal({ ...local, emoji: v })} />
                  </DField>
                </div>
                  ...
                  <div className="mt-3 rounded-xl border p-4 flex items-center justify-between" style={{ borderColor: C.border, background: "rgba(255,255,255,0.02)" }}>
                    <div>
                      ...
                      <div className="text-[11.5px] mt-0.5" style={{ color: C.muted }}>
                        {local.suspended ? "Usuário impedido de acessar o app" : "Usuário com acesso liberado"}
                      </div>
                    </div>
...
// GROUP SIDE DRAWER
// ═══════════════════════════════════════════════════════════════════════════════
function GroupSideDrawer({
  group, users, onClose, onSave, onDelete,
}: {
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-[550] transition-opacity"
        style={{ background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)", opacity: group ? 1 : 0, pointerEvents: group ? "auto" : "none" }}
        onClick={onClose}
      />
      <div
        className="fixed top-0 right-0 h-full z-[560] flex flex-col overflow-y-auto"
        style={{
          ...
          background: "#0f0f15",
          borderLeft: `1px solid ${C.border}`,
          transform: group ? "translateX(0)" : "translateX(100%)",
          transition: "transform 0.32s cubic-bezier(0.32,0.72,0,1)",
        }}
      >
        {local && (
          <>
            ...
            <div className="flex-1 px-6 py-5 space-y-6">
              <div>
                <SectionLabel>Informações</SectionLabel>
                <div className="space-y-3 mt-3">
                  <DField label="Nome do grupo">
                    <DInput value={local.groupName} onChange={v => setLocal({ ...local, groupName: v })} />
                  </DField>
                  <DField label="Emoji">
                    <DInput value={local.groupEmoji ?? ""} onChange={v => setLocal({ ...local, groupEmoji: v })} />
                  </DField>
                  <DField label="Código de convite">
                    <DInput value={local.inviteCode ?? ""} onChange={v => setLocal({ ...local, inviteCode: v })} mono />
                  </DField>
                </div>
                <button onClick={handleSave} disabled={saving}
                  className="mt-4 w-full h-10 rounded-xl text-[13px] font-semibold transition disabled:opacity-50"
  ...
  return (
    <button onClick={onChange} disabled={disabled}
      ...
      style={{
        ...
        borderColor: checked ? C.gold : "rgba(255,255,255,0.2)",
  ...
  return (
    <div className="flex items-center gap-2 h-9 px-3.5 rounded-xl border text-[13px]" style={{ background: C.card, borderColor: "rgba(255,255,255,0.08)" }}>
      ...
      <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        className="bg-transparent outline-none w-44 text-white placeholder:text-[rgba(255,255,255,0.2)]" />
    </div>
  );
}
...
function SidebarBtn({ icon, label, onClick, disabled, spin, red }: { icon: string; label: string; onClick: () => void; disabled?: boolean; spin?: boolean; red?: boolean }) {
  return (
    <button onClick={onClick} disabled={disabled}
      ...
      <i className={`ti ${icon} text-[15px] ${spin ? "animate-spin" : ""}`} />
      {label}
    </button>
  );
}
...
function EmptyRow({ text }: { text: string }) {
  return (
    <div className="px-6 py-10 text-[13px] text-center" style={{ color: C.dim }}>{text}</div>
  );
}
...
function MetaRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex justify-between items-center py-2 border-b" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
      ...
      <span className={`text-[11.5px] ${mono ? "font-mono" : ""} truncate max-w-[200px]`} style={{ color: C.dim }}>{value}</span>
    </div>
  );
}
